...
import pytest
from goods_package.goods_model import Goods
from triangle_model import RightTrianglePair, make_right_triangle_pair


# Tests for RightTrianglePair
def test_right_triangle_pair_init_valid_values() -> None:
    triangle = RightTrianglePair(3, 4)
    assert triangle.first == 3
    assert triangle.second == 4


def test_right_triangle_pair_init_negative_first() -> None:
    with pytest.raises(ValueError, match="Катеты должны быть положительными числами"):
        RightTrianglePair(-3, 4)


def test_right_triangle_pair_init_negative_second() -> None:
    with pytest.raises(ValueError, match="Катеты должны быть положительными числами"):
        RightTrianglePair(3, -4)


def test_right_triangle_pair_init_zero_first() -> None:
    with pytest.raises(ValueError, match="Катеты должны быть положительными числами"):
        RightTrianglePair(0, 4)


def test_right_triangle_pair_init_zero_second() -> None:
    with pytest.raises(ValueError, match="Катеты должны быть положительными числами"):
        RightTrianglePair(3, 0)


def test_right_triangle_pair_hypotenuse_calculation() -> None:
    triangle = RightTrianglePair(3, 4)
    assert triangle.hypotenuse() == 5


def test_right_triangle_pair_hypotenuse_calculation_other_values() -> None:
    triangle = RightTrianglePair(6, 8)
    assert triangle.hypotenuse() == 10


def test_make_right_triangle_pair_valid() -> None:
    triangle = make_right_triangle_pair(6, 8)
    assert triangle.first == 6
    assert triangle.second == 8


def test_make_right_triangle_pair_invalid() -> None:
    with pytest.raises(ValueError, match="Катеты должны быть положительными числами"):
        make_right_triangle_pair(0, 4)


# Tests for Goods
def test_goods_init_valid_values() -> None:
...
def test_goods_change_price_positive() -> None:
    goods = Goods("Товар", "2024-01-15", 100.0, 10, 123)
    goods.change_price(50)
    assert goods.price == 150.0


def test_goods_change_price_negative_valid() -> None:
    goods = Goods("Товар", "2024-01-15", 100.0, 10, 123)
    goods.change_price(-30)
    assert goods.price == 70.0


def test_goods_change_price_negative_invalid() -> None:
    goods = Goods("Товар", "2024-01-15", 50.0, 10, 123)
    with pytest.raises(ValueError, match="Цена не может быть отрицательной"):
        goods.change_price(-60)


def test_goods_change_price_to_zero() -> None:
    goods = Goods("Товар", "2024-01-15", 50.0, 10, 123)
    goods.change_price(-50)
    assert goods.price == 0.0


def test_goods_change_quantity_positive() -> None:
    goods = Goods("Товар", "2024-01-15", 100.0, 10, 123)
    goods.change_quantity(5)
    assert goods.quantity == 15


def test_goods_change_quantity_negative_valid() -> None:
    goods = Goods("Товар", "2024-01-15", 100.0, 10, 123)
    goods.change_quantity(-3)
    assert goods.quantity == 7


def test_goods_change_quantity_negative_invalid() -> None:
    goods = Goods("Товар", "2024-01-15", 100.0, 5, 123)
    with pytest.raises(ValueError, match="Количество не может быть отрицательным"):
        goods.change_quantity(-10)


def test_goods_change_quantity_to_zero() -> None:
    goods = Goods("Товар", "2024-01-15", 100.0, 5, 123)
    goods.change_quantity(-5)
    assert goods.quantity == 0


def test_goods_total_cost() -> None:
    goods = Goods("Товар", "2024-01-15", 100.0, 5, 123)
    assert goods.total_cost() == 500.0


def test_goods_total_cost_zero_quantity() -> None:
    goods = Goods("Товар", "2024-01-15", 100.0, 0, 123)
    assert goods.total_cost() == 0.0


def test_goods_total_cost_zero_price() -> None:
